...
namespace TajimiNow
{
    internal static class EnvVar
    {
        ...
        public static IReadOnlyList<(Regex pattern, string replacement)> RegexReplace { get; } = CsvReader.ReadFromText(
            Environment.GetEnvironmentVariable("MISSKEY_REGEX_REPLACE") ?? string.Empty,
            new() { HeaderMode = HeaderMode.HeaderAbsent, AllowNewLineInEnclosedFieldValues = true }
        ).Select(e => (new Regex(e.Values[0]), e.Values[1])).ToList();
